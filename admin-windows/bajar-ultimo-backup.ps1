param(
    [string]$Servidor = "root@64.176.24.103"
)
...
$remoto = (& ssh -o BatchMode=yes $Servidor "ls -1t /opt/wc3/backups/wc3-backup-*.tar.gz 2>/dev/null | head -1").Trim()
...
& scp -o BatchMode=yes "${Servidor}:$remoto" $local
